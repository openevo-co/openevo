...
export function encryptPersona(plaintext: string, salt: string): Buffer {
...
export function decryptPersona(personaBinary: Buffer): string {
...
export function loadPersonaCartridge(personaFilePath: string): string {
  ...
  try {
    ...
    throw new Error(
      '[DRM] Failed to decrypt persona cartridge. ' +
...
export function findAndLoadPersona(projectPath: string): string | null {
