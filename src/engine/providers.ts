...
async function sendOpenAICompatible(
  ...
  if (!config.apiKey) {
    throw new Error(
      `[OpenEvo] No API key found for provider "${config.provider}". ` +
...
async function sendAnthropic(
  ...
  if (!config.apiKey) {
    throw new Error(
      `[OpenEvo] No API key found for provider "anthropic". ` +
...
async function sendGemini(
  ...
  if (!config.apiKey) {
    throw new Error(
      `[OpenEvo] No API key found for provider "gemini". ` +
...
export function resolveProvider(configYaml: any): ProviderConfig {
  const provider = (configYaml?.provider || 'ollama') as ProviderConfig['provider'];
