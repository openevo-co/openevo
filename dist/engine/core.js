...
import { sendCompletion, resolveProvider } from './providers.js';
import { loadAgentConfig, toProviderConfig } from './agent-loader.js';
...
export class ExoskeletonEngine {
    ...
    async chat(userInput) {
        ...
        if (this.agentConfig.budgetLimit !== undefined) {
            ...
            const recommendation = this.budgetCalculator.recommendModel(this.agentConfig.budgetLimit, estimatedTokens);
            activeProvider = {
                ...activeProvider,
        ...
        this.conversationHistory.push({ role: 'user', content: userInput }, { role: 'assistant', content: response });
        return response;
    }
 ...
 */
export function initEngine(projectPath) {
    // Load .env manually
